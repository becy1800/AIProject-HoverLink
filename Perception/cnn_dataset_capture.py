...
# 3d points
tower2_apex = np.array([lenv.tower2_x, 0, lenv.tower_height])
wire_endpoint = np.array([lenv.tower2_x, -0.9, 2.9])
...
for i in range(num_samples):
    ...
    height_offset = np.random.uniform(lenv.tower_height / 2, lenv.tower_height + 2)
    
    cam_pos = np.array([
        lenv.tower2_x + distance * np.cos(angle),
        distance * np.sin(angle),
        height_offset
    ])
    
    # we point the camera roughly midway between the apex and the goal
    target_center = (tower2_apex + wire_endpoint) / 2.0
    ...
    tx, ty, tc, tu, tv = project_3d_to_2d(tower2_apex, view_mat_np, proj_mat_np, img_width, img_height)
